    ...
    public function test($lesson_slug, Request $request)
    {
        ...
        foreach ($request->get('questions') as $question_id => $answer_id) {
            ...
            if ($correct) {
               ...
               $test = QuestionsOption::where('question_id', $question_id)
                ->where('id', $answer_id)
                ->where('correct', 1)->count() > 0;
            }
        ...
        return redirect()->route('lessons.show', [$lesson->course_id, $lesson_slug])->with('message', 'Điểm: ' . $test);
    }
